...
export default function NewMPIPage() {
  ...
  const [existingMpiNumbers, setExistingMpiNumbers] = useState<string[]>([]);
  ...
  useEffect(() => {
    const initializeData = async () => {
      console.log('🚀 Initializing MPI creation form...');
      ...
      await Promise.all([
        ...
        fetchExistingMpiNumbers(),
      ]);

      // Generate numbers after data is loaded
      const token = localStorage.getItem('token');
      if (validateToken(token)) {
        console.log('🔢 Generating initial numbers...');
        try {
          await generateJobNumber();
          await generateMPINumber();
          console.log('✅ Initial numbers generated successfully');
        } catch (error) {
          console.error('❌ Error generating initial numbers:', error);
        }
      } else {
  ...
  const fetchCustomerCompanies = async () => {
    try {
      ...
      if (response.ok) {
        ...
        console.log(
          '✅ Customer companies set in state:',
          data.customerCompanies?.length || 0,
          'companies'
        );
        if (data.customerCompanies?.length === 0) {
          console.log(
            'ℹ️ No customer companies found. Please add some companies first.'
          );
        }
      } else {
  ...
  const fetchExistingMpiNumbers = async () => {
    try {
      const token = localStorage.getItem('token');
      ...
      if (response.ok) {
        const data = await response.json();
        const numbers: string[] = (data.mpis || [])
          .map((m: any) => m.mpiNumber)
          .filter(Boolean);
        setExistingMpiNumbers(numbers);
      }
    } catch (error) {
      console.error('Error fetching existing MPI numbers:', error);
    }
  };

  const generateJobNumber = async (): Promise<void> => {
    try {
      ...
      if (response.ok) {
        ...
        // Use the job number as-is from the API (already formatted correctly)
        console.log('✅ Setting job number:', data.jobNumber);
        setValue('jobNumber', data.jobNumber);
        console.log(
          '🔍 Job number set in form, current value:',
          watch('jobNumber')
        );
  ...
  const generateMPINumber = async (): Promise<void> => {
    try {
      ...
      if (response.ok) {
        ...
        // Use the MPI number as-is from the API (already formatted correctly)
        console.log('✅ Setting MPI number:', data.mpiNumber);
        setValue('mpiNumber', data.mpiNumber);
        console.log(
          '🔍 MPI number set in form, current value:',
          watch('mpiNumber')
        );
  ...
  const onSubmit = async (data: NewMPIForm) => {
    ...
    const requiredFields = [
      { field: 'jobNumber', name: 'Job Number' },
      { field: 'mpiNumber', name: 'MPI Number' },
      { field: 'mpiVersion', name: 'MPI Revision' },
      { field: 'customerCompanyId', name: 'Customer Company' },
      { field: 'assemblyQuantity', name: 'Assembly Quantity' },
      { field: 'customerAssemblyName', name: 'Customer Assembly Name' },
      { field: 'assemblyRev', name: 'Assembly Revision' },
      { field: 'drawingName', name: 'Drawing Name' },
      { field: 'drawingRev', name: 'Drawing Revision' },
      { field: 'kitReceivedDate', name: 'Kit Received Date' },
      { field: 'dateReleased', name: 'Date Released' },
      { field: 'formId', name: 'Form ID' },
      { field: 'formRev', name: 'Form Revision' },
      { field: 'pages', name: 'Pages' },
    ];

    const missingFields = requiredFields.filter(field => {
      const value = data[field.field as keyof NewMPIForm];
      return (
        !value ||
        value.toString().trim() === '' ||
        value === 'no-companies' ||
    ...
    try {
      ...
      if (!jobNumber || jobNumber.trim() === '') {
        ...
        jobNumber = jobData.jobNumber; // Use as-is from API
      ...
      if (!mpiNumber || mpiNumber.trim() === '') {
        ...
        mpiNumber = mpiData.mpiNumber; // Use as-is from API
  ...
  const selectedCustomerCompany = customerCompanies.find(
    c => c._id === selectedCustomerCompanyId
  );
  ...
  return (
    <div className='min-h-screen gradient-bg dark:gradient-bg'>
      <style jsx global>{`
        /* Force light theme for datalist dropdowns */
        input.light-datalist {
            ...
            <CardContent>
              <form onSubmit={handleSubmit(onSubmit)} className='space-y-6'>
                ...
                <div className={`${showPreview ? 'xl:flex xl:gap-6' : ''}`}>
                  ...
                  <div
                    className={`${showPreview ? 'xl:w-3/5' : 'w-full'} flex flex-col`}
                  >
                      ...
                      <div className='bg-yellow-500/20 border border-yellow-500/30 rounded-lg p-4 mb-6'>
                        ...
                        <div className='flex space-x-2'>
                          <Button
                            ...
                            onClick={() => {
                              console.log(
                                '🧪 Manual test - generating job number'
                              );
                              generateJobNumber();
                            }}
                          ...
                          <Button
                            ...
                            onClick={() => {
                              console.log(
                                '🧪 Manual test - generating MPI number'
                              );
                              generateMPINumber();
                            }}
                          ...
                          <Button
                            ...
                            onClick={() => {
                              console.log(
                                '🧪 Manual test - fetching companies'
                              );
                              fetchCustomerCompanies();
                            }}
                        ...
                        <div className='mt-2 text-sm text-yellow-300'>
                          <p>
                            Forms: {forms.length} | Companies:{' '}
                            {customerCompanies.length}
                          </p>
                          <p>
                            Loading Forms: {isLoadingForms ? 'Yes' : 'No'} |
                            Loading Companies:{' '}
                            {isLoadingCompanies ? 'Yes' : 'No'}
                          </p>
                          <p>
                            Current Job No: {watch('jobNumber') || 'Not set'} |
                            Current MPI No: {watch('mpiNumber') || 'Not set'}
                          </p>
                        </div>
                      </div>
                      ...
                      <div className='space-y-6'>
                        ...
                        <div className='grid grid-cols-1 lg:grid-cols-2 gap-6'>
                          <div className='space-y-2'>
                            ...
                            <Input
                              id='jobNumber'
                              type='text'
                              className={`bg-white/10 border-white/20 text-white ${errors.jobNumber ? 'border-red-500' : ''}`}
                              {...register('jobNumber', {
                          ...
                          <div className='space-y-2 relative'>
                            ...
                            />
                            {showSuggestions && (
                              <div className='absolute z-50 w-full bg-white border border-gray-300 rounded-md shadow-lg max-h-48 overflow-auto'>
                        ...
                        <div className='grid grid-cols-1 lg:grid-cols-2 gap-6'>
                          <div className='space-y-2'>
                            ...
                            <Input
                              ...
                              type='text'
                              list='old-mpi-nos'
                              className={`bg-white/10 border-white/20 text-white ${errors.mpiNumber ? 'border-red-500' : ''}`}
                              {...register('mpiNumber', {
                                required: 'MPI Number is required',
                                minLength: {
                                  value: 3,
                                  message:
                                    'MPI Number must be at least 3 characters',
                                },
                              })}
                            />
                            <datalist id='old-mpi-nos'>
                              {existingMpiNumbers.map(no => (
                          ...
                          <div className='space-y-2'>
                            ...
                            <Input
                              id='mpiVersion'
                              type='text'
                              className={`bg-white/10 border-white/20 text-white ${errors.mpiVersion ? 'border-red-500' : ''}`}
                              {...register('mpiVersion', {
                                required: 'MPI revision is required',
                                minLength: {
                                  value: 1,
                                  message:
                                    'MPI revision must be at least 1 character',
                                },
                              })}
                            />
                            {errors.mpiVersion && (
                              <p className='text-red-400 text-sm'>
                                {errors.mpiVersion.message}
                              </p>
                            )}
                          </div>
                        </div>
                        ...
                        <div className='grid grid-cols-1 lg:grid-cols-2 gap-6'>
                          ...
                          <div className='space-y-2'>
                            ...
                            <Input
                              id='formRev'
                              type='text'
                              className={`bg-white/10 border-white/20 text-white ${errors.formRev ? 'border-red-500' : ''}`}
                              ...
                              {...register('formRev', {
                                ...
                                minLength: {
                                  value: 1,
                                  message:
                                    'Form revision must be at least 1 character',
                                },
                              })}
                            />
                            {errors.formRev && (
                        ...
                        <div className='grid grid-cols-1 lg:grid-cols-2 gap-6'>
                          <div className='space-y-2'>
                            ...
                            <Select
                              onValueChange={value =>
                                setValue('customerCompanyId', value)
                              }
                            >
                              <SelectTrigger
                            ...
                            <input
                              ...
                              {...register('customerCompanyId', {
                                ...
                                validate: value =>
                                  value !== 'no-companies' ||
                                  'Please select a valid customer company',
                              })}
                            />
                            {errors.customerCompanyId && (
                              <p className='text-red-400 text-sm'>
                                {errors.customerCompanyId.message}
                              </p>
                            )}
                          </div>
                          ...
                          <div className='space-y-2'>
                            ...
                            <Input
                              id='assemblyQuantity'
                              type='text'
                              className={`bg-white/10 border-white/20 text-white ${errors.assemblyQuantity ? 'border-red-500' : ''}`}
                              {...register('assemblyQuantity', {
                                required: 'Assembly quantity is required',
                                minLength: {
                                  value: 1,
                                  message:
                                    'Assembly quantity must be at least 1 character',
                                },
                              })}
                            />
                        ...
                        <div className='grid grid-cols-1 lg:grid-cols-2 gap-6'>
                          <div className='space-y-2'>
                            ...
                            <Input
                              ...
                              type='text'
                              className={`bg-white/10 border-white/20 text-white ${errors.customerAssemblyName ? 'border-red-500' : ''}`}
                              {...register('customerAssemblyName', {
                                required: 'Customer assembly name is required',
                                minLength: {
                                  value: 2,
                                  message:
                                    'Customer assembly name must be at least 2 characters',
                                },
                              })}
                            />
                          ...
                          <div className='space-y-2'>
                            ...
                            <Input
                              id='assemblyRev'
                              type='text'
                              className={`bg-white/10 border-white/20 text-white ${errors.assemblyRev ? 'border-red-500' : ''}`}
                              {...register('assemblyRev', {
                                required: 'Assembly revision is required',
                                minLength: {
                                  value: 1,
                                  message:
                                    'Assembly revision must be at least 1 character',
                                },
                              })}
                            />
                            {errors.assemblyRev && (
                        ...
                        <div className='grid grid-cols-1 lg:grid-cols-2 gap-6'>
                          <div className='space-y-2'>
                            ...
                            <Input
                              id='drawingName'
                              type='text'
                              className={`bg-white/10 border-white/20 text-white ${errors.drawingName ? 'border-red-500' : ''}`}
                              {...register('drawingName', {
                                required: 'Drawing name is required',
                                minLength: {
                                  value: 2,
                                  message:
                                    'Drawing name must be at least 2 characters',
                                },
                              })}
                            />
                            {errors.drawingName && (
                          ...
                          <div className='space-y-2'>
                            ...
                            <Input
                              id='drawingRev'
                              type='text'
                              className={`bg-white/10 border-white/20 text-white ${errors.drawingRev ? 'border-red-500' : ''}`}
                              {...register('drawingRev', {
                                required: 'Drawing revision is required',
                                minLength: {
                                  value: 1,
                                  message:
                                    'Drawing revision must be at least 1 character',
                                },
                              })}
                            />
                            {errors.drawingRev && (
                        ...
                        <div className='grid grid-cols-1 lg:grid-cols-2 gap-6'>
                          <div className='space-y-2'>
                            ...
                            <Input
                              id='kitReceivedDate'
                              type='date'
                              className={`bg-white/10 border-white/20 text-white ${errors.kitReceivedDate ? 'border-red-500' : ''}`}
                          ...
                          <div className='space-y-2'>
                            ...
                            <Input
                              id='dateReleased'
                              type='date'
                              className={`bg-white/10 border-white/20 text-white ${errors.dateReleased ? 'border-red-500' : ''}`}
                        ...
                        <div className='space-y-2'>
                          ...
                          >
                            ...
                            <span className='text-white/60 text-sm ml-2'>
                              (Estimated - will be calculated after editing)
                            </span>
                          </Label>
                          <Input
                            id='pages'
                            type='text'
                            placeholder='4'
                            className={`bg-white/10 border-white/20 text-white ${errors.pages ? 'border-red-500' : ''}`}
                            {...register('pages', {
                              required: 'Pages is required',
                              minLength: {
                                value: 1,
                                message: 'Pages must be at least 1 character',
                              },
                            })}
                          />
                          {errors.pages && (
                          ...
                          <div className='text-black'>
                              ...
                              <div className='grid grid-cols-1 md:grid-cols-2 gap-4 text-gray-900'>
                                ...
                                <div>
                                  ...
                                  {customerCompanies.find(
                                    c => c._id === watch('customerCompanyId')
                                ...
                                <div>
                                  ...
                                  {watch('assemblyQuantity') || 'Not selected'}
                                </div>
                                <div>
                                  <span className='font-semibold'>
                                    Customer Assembly Name:
                                  </span>{' '}
                                  {watch('customerAssemblyName') ||
                                    'Not specified'}
                                </div>
                                <div>
                                  <span className='font-semibold'>
                                  ...
                                  {watch('assemblyRev') || 'N/A'}
                                </div>
                                <div>
                                  <span className='font-semibold'>
                                    Drawing Name:
                                  </span>{' '}
                                  {watch('drawingName') || 'N/A'}
                                </div>
                                <div>
                                  <span className='font-semibold'>
                                    Drawing Rev:
                                  </span>{' '}
                                  {watch('drawingRev') || 'N/A'}
                                </div>
                                <div>
                                  <span className='font-semibold'>
                                    Kit receive date:
                                  </span>{' '}
                                  {watch('kitReceivedDate')
                                    ? new Date(
                                        watch('kitReceivedDate')
                                      ).toLocaleDateString()
                                    : 'N/A'}
                            ...
                            <div className='bg-yellow-50 border border-yellow-200 rounded-lg p-4 mt-4'>
                              ...
                              <ul className='text-sm text-yellow-700 space-y-1'>
                                <li>
                                  • This preview shows how your MPI will look
                                  when printed
                                </li>
                                <li>
                                  • All form data will be populated in the final
                                  document
                                </li>
                                <li>
                                  • Process steps will be added in the editor
                                  after creation
                                </li>
                              </ul>
                            </div>
